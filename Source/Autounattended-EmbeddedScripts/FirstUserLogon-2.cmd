...
Set-Variable 'expected=4963532e63884a66ecee0386475ee423ae7f7af8a6c6d160cf1237d085adf05e'

Set-Variable 'onHashErr=download'

Set-Variable 'fileSetup=%~dp0setup.exe'
if exist '%fileSetup%' goto file_check
Set-Variable 'fileSetup=%tmp%\setup.exe'
if exist '%fileSetup%' goto file_check

:file_download
Set-Variable 'onHashErr=error'
...
:file_error
Write-Output File hash does not match the expected value. echo pause exit


REM #Edge
:uninst_edge
Write-Output - Removing Edge
Where-Object /q '%ProgramFiles(x86)%\Microsoft\Edge\Application:*'
if %errorlevel% neq 0 goto uninst_wv
Start-Process /w '' '%fileSetup%' --uninstall --system-level --force-uninstall

REM #WebView
:uninst_wv
Write-Output - Removing WebView
Where-Object /q '%ProgramFiles(x86)%\Microsoft\EdgeWebView\Application:*'
if %errorlevel% neq 0 goto cleanup_wv_junk
Start-Process /w '' '%fileSetup%' --uninstall --msedgewebview --system-level --force-uninstall
...
REM Desktop icon
:users_cleanup
Write-Output - Removing Additional Files

Set-Variable 'REG_USERS_PATH=HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList'
...
Remove-Item /s /q '%1\Desktop\edge.lnk' >NUL 2>&1
Remove-Item /s /q '%1\Desktop\Microsoft Edge.lnk' >NUL 2>&1
...
Remove-Item /s /q '%ProgramFiles(x86)%\Microsoft\Edge' >NUL 2>&1
Remove-Item /s /q '%ProgramFiles(x86)%\Microsoft\EdgeCore' >NUL 2>&1
Remove-Item /s /q '%ProgramFiles(x86)%\Microsoft\EdgeUpdate' >NUL 2>&1
Remove-Item /s /q '%ProgramFiles(x86)%\Microsoft\Temp >NUL 2>&1
